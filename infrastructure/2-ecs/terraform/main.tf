...
resource "aws_ecs_task_definition" "backend" {
  ...
  container_definitions = jsonencode([
    {
      name      = "backend"
      image     = "${data.terraform_remote_state.common.outputs.ecr_backend_url}:latest"
      essential = true
...
resource "aws_ecs_task_definition" "frontend" {
  ...
  container_definitions = jsonencode([
    {
      name      = "frontend"
      image     = "${data.terraform_remote_state.common.outputs.ecr_frontend_url}:latest"
      essential = true
...
resource "aws_lb_target_group" "backend" {
  name        = "${var.project_name}-ecs-backend-tg"
  port        = 8080
  protocol    = "HTTP"
...
resource "aws_lb_target_group" "frontend" {
  name        = "${var.project_name}-ecs-frontend-tg"
  port        = 80
  protocol    = "HTTP"
  vpc_id      = data.terraform_remote_state.common.outputs.vpc_id
  target_type = "ip"
